...
from mydatasets import SentenceGenCLSWeightDataset
...
parser.add_argument("--sentence_file_path", default="/home/yutao_zhu/BertModel/", type=str, help="The path to save log.")
...
print(args)
predict_data = args.sentence_file_path
tokenizer = BertTokenizer.from_pretrained(args.model_path)

def predict(model, X_test):
    model.eval()
    test_dataset = SentenceGenCLSWeightDataset(X_test, 512, tokenizer, args.dataset_script_dir, args.dataset_cache_dir)
    ...
    for i in tqdm(range(len(attention_weights))):
        sid = test_dataset.passage_dataset[i]['sid']
        stext = test_dataset.passage_dataset[i]['sentence_tokens']
        anos =  test_dataset.passage_dataset[i]['ano']
        pnos =  test_dataset.passage_dataset[i]['pno']
        slen = len(stext)
        cls_attention = attention_weights[i][1:slen+1]
        if len(cls_attention) != slen:
            stext = stext[:500]
            cls_attention =  attention_weights[i][1:500+1]
        att_text = []
        for st, sa in zip(stext, cls_attention):
            att_text += [[st, sa]]
        fw.write(json.dumps({"att_text": att_text, "sid":sid, "ano":anos, "pno":pnos, "sentence_tokens":stext}, ensure_ascii=False)+"\n")
    fw.close()
